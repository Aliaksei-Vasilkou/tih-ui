...
- ✅ **Always:** Follow the implementation checklist order (types → API → store → components → query wiring). Use semantic Tailwind colour tokens only — these are the colour/theming classes defined in `tailwind.config.js` (e.g. `bg-surface`, `text-foreground`, `border-muted`). Utility classes for layout, spacing, typography, and borders (e.g. `flex`, `p-4`, `rounded-lg`, `font-medium`) are allowed.
