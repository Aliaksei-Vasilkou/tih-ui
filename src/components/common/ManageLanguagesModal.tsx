...
export default function ManageLanguagesModal({ onClose }: Props) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: LanguageCreateRequest }) => languagesApi.update(id, data),
    onSuccess: () => {
      invalidate();
      resetForm();
    },
    ...
    <ModalShell title="Manage Languages" onClose={onClose} footer={footer}>
      <div className="px-6 py-4 space-y-1">
        {(() => {
          ...
          if (languages.length === 0 && !showAdd) {
            return <p className="text-center text-muted-light py-8 text-sm">No languages yet.</p>;
          }
          return (
