...
export default function ManageTagsModal({ initialLanguageId, onClose }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: TagCreateRequest) => tagsApi.create(selectedLanguageId as number, data),
    onSuccess: () => {
      invalidate();
      resetForm();
    },
  ...
  return (
    <ModalShell title="Manage Tags" onClose={onClose} footer={footer}>
      ...
      <div className="px-6 py-2 space-y-1">
        {(() => {
          if (!selectedLanguageId) {
            return <p className="text-center text-muted-light py-8 text-sm">Select a language to see its tags.</p>;
          }
          if (isLoading) {
            return (
          ...
          if (tags.length === 0 && !showAdd) {
            return <p className="text-center text-muted-light py-8 text-sm">No tags for this language yet.</p>;
          }
          return (
