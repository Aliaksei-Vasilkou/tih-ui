...
export default function ManageCategoriesModal({ initialLanguageId, onClose }: Props) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: CategoryCreateRequest }) => categoriesApi.update(id, data),
    onSuccess: () => {
      invalidate();
      resetForm();
    },
  ...
  const handleSave = () => {
    ...
    if (!selectedLanguageId) {
      setFormError('Select a language first');
      return;
    }
  ...
  return (
    <ModalShell title="Manage Categories" onClose={onClose} footer={footer}>
      <div className="px-6 pt-4 pb-2">
        <label className="block text-xs font-medium text-muted mb-1 uppercase tracking-wide">Language</label>
        <Select
      ...
      <div className="px-6 py-2 space-y-1">
        {(() => {
          if (!selectedLanguageId) {
            return (
              <p className="text-center text-muted-light py-8 text-sm">Select a language to see its categories.</p>
            );
          }
          if (isLoading) {
            return (
          ...
          if (categories.length === 0 && !showAdd) {
            return <p className="text-center text-muted-light py-8 text-sm">No categories for this language yet.</p>;
          }
          return (
